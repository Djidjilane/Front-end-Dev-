...
const SidebarMenu = ({ isOpen, toggleMenu, activeMenu, handleMenuClick }) => {
  ...
  return (
    <>
      ...
      <div
        className={`fixed lg:absolute lg:top-0 top-0 lg:left-0 left:0 lg:w-1/4 w-full h-[100vh] bg-gray-800 text-white z-50 navbar-menu scrollbar-thin scrollbar-thumb-[#15803D] scrollbar-track-[#f1f1f1] p-4 overflow-y-auto border border-gray-300 rounded-md ${isOpen ? "block" : "hidden"}`}
      >
        ...
        <div className="h-full">
          ...
          <div className="flex items-center justify-between mt-4">
            <input
              ...
              className="flex-1 p-2 border border-gray-300 text-black rounded-md  focus:outline-none"
            />
            {/* <button onClick={toggleMenu} className="ml-2 text-white">
              <svg
                xmlns="http://www.w3.org/2000/svg"
          ...
          {/* Liste des jeux */}
          <nav className="flex flex-col bg-gray-800 my-8 space-y-4 h-[90vh]">
            ...
            {filteredGames.length > 0 ? (
              filteredGames.map((game) => (
                <a
                  key={game.id}
                  href={`/${game.id}`}
                  className={`flex items-center rounded-md bg-white hover:bg-[#15803D] hover:text-white text-black p-3 space-x-2 ${activeMenu === game.id ? "text-[#15803D]" : "text-black/60"}`}
