// src/v1/pages/Jobs/OffresStagesEntreprise.jsx
import { useState, useEffect } from "react";
import axios from "axios";

const OffresStagesEntreprise = () => {
  const [offres, setOffres] = useState([]);

  useEffect(() => {
    axios.get("http://localhost:8000/api/offres") // adapte l’URL à ton backend
      .then((response) => {
        const stages = response.data.filter((offre) => offre.type === "Stage");
        setOffres(stages);
      })
      .catch((error) => {
  ...
  return (
    <div className="p-6 space-y-4">
      <h2 className="text-2xl font-semibold text-gray-800 border-b pb-2">📄 Offres de stages</h2>
...
export default OffresStagesEntreprise;
