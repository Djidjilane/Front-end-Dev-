  ...
  const fetchCandidatures = async () => {
    try {
      ...
      const response = await axiosInstance.get(
        `/entreprise/candidatureParOffreStage/${id}`,
        {
        ...
        <div className="max-w-4xl mx-auto">
          <div className="mb-8">
            <div className="flex justify-between items-start">
              <div>
                <h1 className="text-2xl font-bold text-gray-800">
                  Candidatures pour : <span className="text-blue-600">{offre?.domaine || "Offre #" + id}</span>
                </h1>
                <p className="text-gray-600 mt-1">
                  {offre?.niveau && `Niveau requis : ${offre.niveau}`}
                </p>
              </div>
              <button
                onClick={() => navigate("/entreprise/offres-stage")}
            ...
            <div className="mt-4 bg-blue-50 p-3 rounded-md">
              <p className="text-sm text-blue-800">
                <span className="font-medium">{candidatures.length}</span> candidature(s) reçue(s)
              </p>
            </div>
          </div>
          ...
          {candidatures.length === 0 ? (
            <div className="bg-white p-6 rounded-lg shadow-sm text-center">
              ...
              <p className="mt-1 text-sm text-gray-500">Aucun candidat n'a postulé à cette offre de stage pour le moment.</p>
            </div>
          ) : (
                  ...
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <div className="flex flex-wrap gap-x-4 gap-y-2 mt-2 text-sm text-gray-600">
                        ...
                        <span className={`flex items-center px-2 py-1 rounded-full text-xs font-medium ${getStatusBadgeClass(candidat.statut)}`}>
