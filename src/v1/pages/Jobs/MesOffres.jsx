...
export default function MesOffresEmploi() {
  ...
  return (
    <div className="p-4">
      <h1 className="text-xl font-bold mb-4">Mes Offres d'Emploi</h1>
      {offres.length === 0 ? (
        ...
        <ul className="space-y-3">
          {offres.map((offre) => (
            <li key={offre.id} className="p-4 border rounded-md shadow-sm">
              <h2 className="text-lg font-semibold">{offre.projet || 'Titre non défini'}</h2>
              <p>{offre.description || 'Pas de description'}</p>
              <p className="text-sm text-gray-500">Lieu : {offre.lieu || 'Inconnu'}</p>
